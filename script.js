...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('https://raw.githubusercontent.com/jbrooksuk/JSON-Airports/master/airports.json')
        ...
        .then(data => {
            ...
            const globalAirports = data.filter(item => item.iata && item.iata !== "" && !popularCodes.includes(item.iata)).map(item => {
                return { cityKo: item.city||item.name||"알 수 없는 도시", cityAlias: '', cityEn: item.city||item.name||"Unknown", code: item.iata, airportKo: item.name||"Unknown", airportEn: item.name||"Unknown" };
            });
    ...
    const setupAutocomplete = (inputEl, dropdownEl) => {
        ...
        inputEl.addEventListener('input', () => {
            ...
            const filtered = combinedAirports.filter(item => 
                isMatch(item.cityKo) || isMatch(item.cityEn) || isMatch(item.cityAlias) || isMatch(item.airportKo) || isMatch(item.code)
            ).slice(0, 10); 
    ...
    const showLoadingThenResult = (searchType) => {
        ...
        if(searchType === 'flight') {
            icon.innerText = 'flight'; text.innerText = '최저가 항공권을 찾고 있어요';
        } else {
            icon.innerText = 'bed'; text.innerText = '인기 숙소를 찾고 있어요';
        }
        ...
        for(let i=0; i<4; i++) {
            ...
            <div class="ticket-card">
                <div class="ticket-leg">
                    <div class="airline-info"><div class="airline-logo" style="background:#0064DE">KE</div><span class="airline-name">대한항공</span></div>
                ...
                <div class="ticket-footer">
                    <span class="ticket-price">${price.toLocaleString()}원</span><button class="select-btn ripple-btn">선택하기</button>
                </div>
            </div>`;
        }
        dateTabsContainer.style.display = 'flex'; // 탭 보이기
        resultListContainer.innerHTML = html; bindRipple(); 
        showLoadingThenResult('flight');
    });
    ...
    document.getElementById('btn-search-hotel')?.addEventListener('click', () => {
        ...
        document.getElementById('result-route-text').innerText = `${dest} 숙소 검색결과`;
        document.getElementById('result-date-text').innerText = `${fm(hotelStartDate)} ~ ${fm(hotelEndDate)} · 성인 ${guestData.adult}명`;
        
        const hotels = [
            { name: "호텔 그레이서리 신주쿠", rating: 8.8, rev: 1204, img: "url('https://images.unsplash.com/photo-1551882547-ff40c0d1398c?q=80&w=400&auto=format&fit=crop')" },
            { name: "신주쿠 워싱턴 호텔", rating: 8.2, rev: 950, img: "url('https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=400&auto=format&fit=crop')" },
            { name: "게이오 플라자 호텔", rating: 9.1, rev: 2100, img: "url('https://images.unsplash.com/photo-1582719508461-905c673771fd?q=80&w=400&auto=format&fit=crop')" }
        ];
        ...
        hotels.forEach(h => {
            ...
            <div class="hotel-card">
                <div class="hotel-img" style="background-image: ${h.img}; background-size: cover; background-position: center;">
                    <div class="hotel-rating"><span class="material-symbols-rounded" style="font-size:14px; color:#FBBF24;">star</span> ${h.rating}</div>
                </div>
                <div class="hotel-info">
                    <h3 class="hotel-name">${h.name}</h3>
                    <div class="hotel-loc"><span class="material-symbols-rounded" style="font-size:16px;">location_on</span> 도심에서 0.5km · 리뷰 ${h.rev}개</div>
                    <div class="hotel-price-row">
                        <span class="h-price-label">1박 기준, 세금 포함</span>
                        <span class="h-price-val">${price.toLocaleString()}원</span>
                    </div>
                </div>
            </div>`;
        });
        dateTabsContainer.style.display = 'none'; // 호텔 결과에선 날짜 탭 숨기기
        resultListContainer.innerHTML = html; 
        showLoadingThenResult('hotel');
    });
